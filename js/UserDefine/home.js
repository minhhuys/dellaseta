var Home = {
    ...
    initHome: function () {
        ...
        client.search(searchAddress, function (err, data) {
            ...
            $.each(data.hits.hits, function (i, value) {
                ...
                build += "<h3>" + value._source.name + "</h3>";
                build += '<div class="text">';
                build += value._source.detailAddress + '</br>';
                build += 'E-mail: <a href = "mailto:' + value._source.email + '">' + value._source.email + '</a></br>';
                build += 'Số điện thoại: <a href="tel:' + value._source.mobile + '">' + value._source.mobile + '</a></br>';
                build += '</div>';

            });
            $("#divAddress").html(build);
        });
        ...
        client.search(searchParams, function (err, data) {
            $.each(data.hits.hits, function (i, value) {
                ...
                if (value._source.displayOrder === ConfigHome.home5) {
                    $('#section_home5 h2').html(value._source.description);
    ...
    buildHome7(data) {
        ...
        build += '      <a href="#" class="theme-btn btn-style-two">Nhận E-Voucher</a>';
        build += '</div>';
        return build;
    },
    ...
    buildHome4(data) {
        ...
        build += '                        <h3><a href="/' + RewriteUrl(data.name) + '/' + data.id + '" title="' + data.name + '">' + substringInSpace(data.name, 20, '...') + '</a></h3>';
        build += '                  </div>';
        build += '                  <div style="position:absolute;top:0;right:-2px;" class="pull-right">';
        build += '                        <div class="price">' + data.price.formatMoney(0, 3) + ' vnđ</div>';
        build += '                  </div>';
        build += '            </div>';
        build += '            <div class="text">' + substringInSpace(data.subDescription, 55, '...') + '</div>';
        build += '      </div>';
        build += '</div>';
        return build;
    },
    ...
    buildBannerHome(data, index) {
        ...
        build += '            <h2 class="text-center">' + data.subDescription + '</h2>';
        build += '      </div>';
